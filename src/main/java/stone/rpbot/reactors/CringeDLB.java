...
public class CringeDLB implements Job {
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		long cutOffTime = TimeUtil
				.getDiscordTimestamp(LocalDateTime.of(nowDate, pastTime).toInstant(ZoneOffset.of("-4")).toEpochMilli());
		...
		{
			if (message.getIdLong() < cutOffTime)
				break;
			if (message.getAuthor().getIdLong() == DLB_USERID)
			{
				// SharedConstants.GLOBAL_LOGGER.debug(message.getContentRaw());
				if (!message.getEmbeds().isEmpty()
						|| message.getContentRaw().equals("The queue is empty! More lyrics need to be added."))
				{
					foundLyrics = true;
